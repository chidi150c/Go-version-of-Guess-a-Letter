...
func (h *UserHandler) signupPostHandler(w http.ResponseWriter, r *http.Request) {
	...
	_, err := h.Session.Userservice.GetUser(msg.Username) //get user with username and check if it exist
	if err == nil || err == data.ErrUserNameEmpty || msg.Validate() == false {
		if err == nil {
			msg.Errors["Username"] = fmt.Sprintf("Username \"%s\" is already taken, please try another", msg.Username)
			_ = msg.Validate()
		} else if err == data.ErrUserNameEmpty {
			msg.Errors["Username"] = data.ErrUserNameEmpty
			_ = msg.Validate()
		}
	...
	if err := h.Session.Userservice.AddUser(usr); err != nil {
		tools.Error(w, err, http.StatusInternalServerError, h.Logger)
		return
	}
...
func (h *UserHandler) loginPostHandler(w http.ResponseWriter, r *http.Request) {
	...
	msg.Errors = make(map[string]interface{})
	// username taken or password or email incorrect ?
	user, err := h.Session.Userservice.GetUser(msg.Username) //get user with username and check if it exist
	if err != nil {
...
func (h *UserHandler) handleListUsers(w http.ResponseWriter, r *http.Request) {
	...
	if user.Level == "Admin" {
		...
		usrs, err := h.Session.Userservice.ListUsers()
		if err != nil {
			tools.Error(w, err, http.StatusInternalServerError, h.Logger)
			return
		}
		dat := struct {
			Usrs       data.Mmm //[]*data.User
			AddUserURL string
		}{
			Usrs:       data.Mmm(usrs),
...
func (h *UserHandler) handleGetUser(w http.ResponseWriter, r *http.Request) {
	...
	// Find user by ID.
	d, err := h.Session.Userservice.GetUser(username)
	if err != nil {
		tools.Error(w, err, http.StatusInternalServerError, h.Logger)
	} else if d == nil {
		tools.NotFound(w)
